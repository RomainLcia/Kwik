...
const s = StyleSheet.create({
  ...
  ibanBlock: { marginTop: 16, paddingTop: 10, borderTopWidth: 0.5, borderTopColor: border },
  ibanLabel: { fontSize: 7, color: light, textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 4 },
  ibanText: { fontSize: 9, color: dark, fontFamily: 'Helvetica-Bold' },
  condBlock: { marginTop: 14, paddingTop: 10, borderTopWidth: 0.5, borderTopColor: border },
...
export function InvoiceDocument({ invoice, lines, company, client }: InvoiceDocumentProps) {
  const vatBreakdown = lines.reduce((acc, line) => {
    if (Number(line.line_total_ht) === 0) return acc
    const rate = line.vat_rate
    const base = Number(line.line_total_ht) * (1 - Number(invoice.discount_percent) / 100)
      ...
      <Page size="A4" style={s.page}>
        ...
        <View style={s.row}>
          ...
          <View style={s.headerRight}>
            ...
            <Text style={s.docMeta}>Emise le {fmtDate(invoice.issue_date)}</Text>
            <Text style={s.docMeta}>Echeance le {fmtDate(invoice.due_date)}</Text>
          </View>
        </View>
        ...
        {invoice.object ? (
          <View style={s.objectBlock}>
            ...
            <Text style={s.objectText}>{invoice.object}</Text>
          </View>
        ) : null}
        ...
        <View style={s.totalsSection}>
          <View style={s.totalsInner}>
            <View style={s.totalsRow}>
              ...
              <Text style={s.tVal}>{fmt(Number(invoice.subtotal_ht))}</Text>
            </View>
            {Number(invoice.discount_percent) > 0 ? (
              <View style={s.totalsRow}>
                <Text style={s.tLabel}>Remise ({invoice.discount_percent}%)</Text>
                <Text style={s.tValGreen}>- {fmt(Number(invoice.discount_amount))}</Text>
              </View>
            ) : null}
            <View style={s.thinDivider} />
            <View style={s.totalsRow}>
              <Text style={s.tLabel}>Total HT</Text>
              <Text style={s.tVal}>{fmt(Number(invoice.total_ht))}</Text>
            </View>
        ...
        {company.iban ? (
          <View style={s.ibanBlock}>
            <Text style={s.ibanLabel}>Virement bancaire</Text>
            <Text style={s.ibanText}>IBAN : {company.iban}</Text>
          </View>
        ) : null}
          ...
          <View style={s.condBlock}>
            {invoice.terms ? (
              <View style={s.condSection}>
                ...
                <Text style={s.condText}>{invoice.terms}</Text>
              </View>
            ) : null}
            {invoice.notes ? (
              <View style={s.condSection}>
                <Text style={s.condLabel}>Notes</Text>
                <Text style={s.condText}>{invoice.notes}</Text>
              </View>
            ) : null}
          </View>
        ) : null}
